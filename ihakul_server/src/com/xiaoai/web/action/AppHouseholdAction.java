...
public class AppHouseholdAction extends XiaoaiMessage{
	 ...
	 * @param props 通讯参数，见链路协议 
	 * @param stub 智能索引(Key)
	 * @param groupbbh 家庭组版本号
	...
	public String insert() throws IOException{
		...
		if(success){
			...
			if(users !=null ){
				family=familyDao.getFamilygroupByNumber(Integer.parseInt(groupNumber));
				if(family!=null){	
				 ...
				 if(userId.equals(fu1.getDna())){ //判断该家庭组是不是该用户创建		
					List<Household> kk=householdDao.getRoomByRoomNumber1(family,eaNumber);
					...
					households.setClassId(Integer.parseInt(classId));
					households.setEaName(eaName);
					households.setEaNumber(eaNumber);
					households.setCreateTime(XATools.getTNowTime());
					success=houseHoldService.insertHousehold(households);
					if(success){
...
//						array.add(json2);
	...
	public String findAll() throws IOException{
		...
		if(total !=0){//如果该家庭组下有家电信息
			...
			List<Household> houseList=houseHoldService.findHouseHold(total, offset, houseHolds);
		...
		if(success){
		  ...
		  if(users !=null){
			...
			if(family!=null){	
			   ...
			   if(fu1!=null){
				 if(userId.equals(fu1.getDna())){ //判断该家庭组是不是该用户创建	
				   Household houseHold=householdDao.getRoomByRoomNumber(eaNumber);
				   if(houseHold!=null){
				   success=householdDao.deleteHousehold(houseHold);
					if(success==false){
	...
	public String selectByid() throws IOException{
		...
		if(success){
			...
			List<Household> lh=householdDao.getRoomByRoomNumber1(family,eaNumber);
